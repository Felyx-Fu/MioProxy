...
export function LogsPage() {
  ...
  return (
    <section className="page-stack logs-page">
      ...
      <div className="terminal-card"><div className="terminal-topbar"><span><i /> <i /> <i /></span><small>{paused ? "STREAM PAUSED" : "MIHOMO /logs · LIVE"}</small><b>{visible.length} lines</b></div><div className="terminal-body">{visible.length === 0 ? <div className="terminal-empty">等待 Mihomo 日志流…</div> : visible.map((entry, index) => <div className="log-line" key={`${entry.timestamp}-${index}`}><time>{timeLabel(entry.timestamp)}</time><span className={`log-level ${entry.level.toLowerCase()}`}>{entry.level}</span><p>{entry.message}</p></div>)}<div ref={bottomRef} /></div></div>
    </section>
  );
}
