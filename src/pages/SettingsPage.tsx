...
export function SettingsPage(props: SettingsPageProps) {
  ...
  const content: Record<Category, Array<{ search: string; node: ReactNode }>> = useMemo(() => ({
    ...
    network: [
      ...
      { search: "tun transparent route network 透明代理", node: <SettingRow key="tun" icon={<Route size={16} />} title={t("settings.network.tun.title")} description={tunExternal ? t("settings.network.tun.externalDescription") : tunStatus?.projection === "waitingForService" ? t("settings.service.reconnectingWarning") : tunStatus?.projection === "recovering" ? t("settings.network.tun.recoveryPending") : t("settings.network.tun.description")} value={!tunStatus ? t("common.checking") : tunExternal || tunStatus.projection === "external" ? t("common.external") : tunStatus.projection === "on" ? t("common.on") : tunStatus.projection === "off" ? t("common.off") : tunStatus.projection === "error" ? t("common.error") : tunStatus.projection === "recovering" ? t("settings.network.tun.recoveryPending") : tunStatus.projection === "waitingForService" ? t("settings.service.reconnecting") : t("settings.network.tun.transitioning")} control={<button className="setting-toggle" type="button" onClick={onRequestTunTransition} disabled={tunBusy || tunExternal || tunStatus?.projection === "waitingForService" || tunStatus?.projection === "recovering" || tunStatus?.projection === "enabling" || tunStatus?.projection === "disabling" || (!tunWillDisable && coreState !== "ready")}>{tunBusy ? t("common.working") : tunExternal ? t("common.external") : tunWillDisable ? t("settings.action.turnOff") : t("settings.action.turnOn")}</button>} /> },
