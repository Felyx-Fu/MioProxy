...
export function RulesPage({ running }: { running: boolean }) {
  ...
  return (
    <section className="page-stack rules-page">
      ...
      {error && <div className="error-banner" role="alert"><Workflow size={17} /><span>{error}</span></div>}
      {message && <div className="success-banner" role="status"><Database size={17} /><span>{message}</span></div>}
      ...
      {!running ? <div className="empty-card empty-card-large panel"><Workflow size={28} /><strong>{t("rules.coreNotReady")}</strong><p>{t("rules.coreNotReadyDescription")}</p></div> : <>
        ...
        <div className="rules-toolbar panel"><label className="search-box"><Search size={16} /><input data-page-search value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("rules.searchPlaceholder")} aria-label={t("rules.searchPlaceholder")} /></label><label className="select-field"><select value={type} onChange={(event) => setType(event.target.value)} aria-label={t("rules.type")}><option value="all">{t("rules.allTypes")}</option>{types.map((item) => <option value={item} key={item}>{item}</option>)}</select></label></div>
