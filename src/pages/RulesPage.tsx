  ...
  return (
    <section className="page-stack rules-page">
      <header className="page-header compact-header">
        <div><p className="eyebrow">RULES / MATCHING</p><h1>Rules</h1><p>Inspect configured rule order and Rule Provider state without editing subscription sources.</p></div>
        <button className="secondary-button" type="button" onClick={() => void load()} disabled={!running || loading}><RefreshCw size={16} className={loading ? "spin" : ""} />Refresh</button>
      </header>
      ...
      {!running ? <div className="empty-card empty-card-large panel"><Workflow size={28} /><strong>Core 尚未 Ready</strong><p>Core Ready 后，当前规则和 Rule Provider 会从 Controller API 加载。</p></div> : <>
        ...
        <div className="rules-toolbar panel"><label className="search-box"><Search size={16} /><input data-page-search value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search rules or policies…" /></label><select value={type} onChange={(event) => setType(event.target.value)}><option value="all">All types</option>{types.map((item) => <option value={item} key={item}>{item}</option>)}</select></div>

        <section className="rules-section"><div className="section-heading"><div><span>CONFIGURED ORDER</span><strong>Rule chain</strong></div><small>{visible.length} / {rules.length}</small></div><div className="rules-table-wrap panel"><table className="rules-table"><thead><tr><th>#</th><th>Type</th><th>Payload</th><th>Policy</th></tr></thead><tbody>{visible.length === 0 ? <tr><td colSpan={4} className="table-empty">No rules match the current filters.</td></tr> : visible.map(({ rule, index }) => <tr key={`${index}-${rule.type}-${rule.payload}`}><td className="rule-index">{index + 1}</td><td><span className="rule-type">{String(rule.type ?? "UNKNOWN")}</span></td><td><strong>{String(rule.payload || "—")}</strong>{rule.subRules?.length ? <small>{rule.subRules.join(" → ")}</small> : null}</td><td><span className="policy-pill">{rulePolicy(rule)}</span></td></tr>)}</tbody></table></div></section>
