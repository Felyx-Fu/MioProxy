...
export function OverridesPage({ profileId }: { profileId: string | null }) {
  ...
  return (
    <section className="page-stack overrides-page">
      ...
      <div className="config-flow panel"><span className="flow-step active">1 <b>{t("overrides.flowSubscriptionProfile")}</b></span><i>+</i><span className="flow-step active">2 <b>{t("overrides.flowLocalOverride")}</b></span><i>→</i><span className="flow-step">3 <b>{t("overrides.flowConfigBuilder")}</b></span><i>→</i><span className="flow-step">4 <b>{t("overrides.flowValidateReload")}</b></span></div>

      <div className="override-layout"><section className="override-editor-card panel"><div className="editor-heading"><div><span>LOCAL-OVERRIDE.YAML</span><strong>{t("overrides.localLayer")}</strong></div><FileCode2 size={18} /></div><textarea className="override-editor" value={content} onChange={(event) => setContent(event.target.value)} placeholder={t("overrides.example")} spellCheck={false} /><div className="editor-footer"><span>{t("overrides.storageNote")}</span><div><button className="secondary-button" type="button" onClick={() => void save()} disabled={saving || applying}><Save size={16} />{saving ? t("overrides.saving") : t("overrides.saveOverride")}</button><button className="secondary-button" type="button" onClick={() => void previewConfig()} disabled={loading || saving || applying}><Eye size={16} />{t("overrides.previewButton")}</button><button className="primary-button" type="button" onClick={() => void apply()} disabled={applying || saving}><ShieldCheck size={16} />{applying ? t("overrides.applying") : t("overrides.validateApply")}</button></div></div></section>
